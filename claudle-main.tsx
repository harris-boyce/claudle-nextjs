...
const ClaudLE = () => {
  ...
  const [personality, setPersonality] = useState('lasso');
  ...
  const LoadingSpinner = ({ text, icon: Icon = Loader2 }) => (
  ...
  const getNewWord = async () => {
    ...
    try {
      ...
      const excludeText = usedWordsArray.length > 0 
        ? `Do NOT use any of these words that have already been used: ${usedWordsArray.join(', ')}.` 
        : '';

      const prompt = `Generate exactly one 5-letter word for a Wordle game with the theme: "${themes[theme].description}".

Requirements:
- Exactly 5 letters
- Uses only standard English alphabet (A-Z)
- Must be a real, common word that players would know
- Appropriate for the theme: ${themes[theme].description}
${excludeText}
...
Respond with ONLY the word in uppercase letters, nothing else.`;
  ...
  const analyzeCurrentGuess = async (guess) => {
...
The player is playing ClaudLE (theme: "${themes[theme].description}") and is considering the guess "${guess}" for their ${guessCount + 1}th attempt.
...
Analysis of their potential guess "${guess}":
...
Keep it under 100 words and stay in character. Be encouraging but educational.`;
      ...
      const data = await response.json();
      setCoachingText(data.content[0].text);
    } catch (error) {
  ...
  }, [currentGuess, interactiveCoach, gameState, guesses, targetWord]);
...
The player is playing ClaudLE with the theme "${themes[theme].description}". They're on guess ${guessCount} of 6.

Target word: ${targetWord}
Their guesses so far: ${guesses.join(', ')}
Letters not in the word: ${wrongLetters.join(', ') || 'None identified yet'}
...
Give them a helpful hint without revealing the answer. Make the hint more specific as they get closer to guess 6. Stay in character!`;
      ...
      const data = await response.json();
      setHintText(data.content[0].text);
    } catch (error) {
  ...
  const getGameOverMessage = async (won) => {
...
The player just ${won ? 'won' : 'lost'} a ClaudLE game. The word was "${targetWord}" and they took ${guesses.length} guesses.
...
Keep it short and stay in character!`;
      ...
      const data = await response.json();
      setGameOverMessage(data.content[0].text);
    } catch (error) {
